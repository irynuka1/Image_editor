...
def run_test(task, test, use_valgrind=False, ):
    ...
    if child.returncode != 0:
        if not use_valgrind:
            log('- UPS: exit code = {} '.format(child.returncode))
...
def check_test(task, test):
    ...
    if child.returncode != 0:
        log(
            '- WA: {} '.format(extract_stderr(child)))
        unindent_log()
        return child.returncode
...
def run_task(config, task):
    ...
    log('----- running task {} - grade {:g}/{:g} '.format(task.id,
